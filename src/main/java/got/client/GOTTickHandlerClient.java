...
public class GOTTickHandlerClient {
	...
	public static int balrogNearTickMax = 100;
	...
	public double lastUtumnoReturnY = -1.0;
		...
		block74: {
			...
			block75: {
				...
				block76: {
					block77: {
						...
						if (minecraft.renderViewEntity.isPotionActive(Potion.confusion.id)) {
							...
							utumnoCamRoll = 0.0f;
							GOTReflectionClient.setCameraRoll(minecraft.entityRenderer, utumnoCamRoll);
						}
						if (newDate > 0) {
							--newDate;
						}
				...
				if (inUtumnoReturnPortal) {
					entityplayer.setPosition(utumnoReturnX + 0.5, entityplayer.posY, utumnoReturnZ + 0.5);
					if (lastUtumnoReturnY >= 0.0 && entityplayer.posY < lastUtumnoReturnY) {
						entityplayer.setPosition(entityplayer.posX, lastUtumnoReturnY, entityplayer.posZ);
					}
					lastUtumnoReturnY = entityplayer.posY;
				} else {
					lastUtumnoReturnY = -1.0;
				}
	...
	public void renderAlignment(Minecraft mc, float f) {
		...
		if (alignDrainTick > 0 && text) {
			...
			GOTTickHandlerClient.renderAlignmentDrain(mc, (int) alignmentXF - 155, (int) alignmentYF + 2, alignDrainNum, alpha);
		}
	}
	...
	public static void renderAlignmentDrain(Minecraft mc, int x, int y, int numFactions) {
		GOTTickHandlerClient.renderAlignmentDrain(mc, x, y, numFactions, 1.0f);
	}
	...
	public static void renderAlignmentDrain(Minecraft mc, int x, int y, int numFactions, float alpha) {
		...
		GL11.glColor4f(1.0f, 1.0f, 1.0f, alpha);
		mc.getTextureManager().bindTexture(GOTClientProxy.alignmentTexture);
		GOTTickHandlerClient.drawTexturedModalRect(x, y, 0, 128, 16, 16);
		GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		String s = "-" + numFactions;
		FontRenderer fr = mc.fontRenderer;
		GOTTickHandlerClient.drawBorderedText(fr, x + 8 - fr.getStringWidth(s) / 2, y + 8 - fr.FONT_HEIGHT / 2, s, 16777215, alpha);
		GL11.glDisable(3042);
	}
