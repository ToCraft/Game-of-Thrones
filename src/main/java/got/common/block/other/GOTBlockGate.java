...
public class GOTBlockGate extends Block implements GOTConnectedBlock {
	...
	public void addAdjacentGates(World world, int i, int j, int k, int dir, boolean open, int iter, Map<ChunkCoordinates, Integer> coordsList) {
		if (dir != 0 && dir != 1) {
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
		...
		if (itemstack != null) {
			...
			if (Block.getBlockFromItem(item) instanceof GOTBlockGate) {
				return false;
			}
	...
	public void onNeighborBlockChange(World world, int i, int j, int k, Block block) {
		if (!world.isRemote && !(block instanceof GOTBlockGate)) {
			...
			if ((powered || block.canProvidePower()) && (powered ? !open : open)) {
				activateGate(world, i, j, k);
			}
		}
	}

	public void recursiveAddGates(World world, int i, int j, int k, int dir, boolean open, int iter, Map<ChunkCoordinates, Integer> coordsList) {
