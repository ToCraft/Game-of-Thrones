...
public class GOTEventHandler implements IFuelHandler {
	...
	public void onBlockInteract(PlayerInteractEvent event) {
		...
		if (event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
			...
			if (!mightBeAbleToAlterWorld) {
				...
				} else if ((block instanceof GOTBlockArmorStand) || (block instanceof GOTBlockWeaponRack || block == Blocks.bookshelf)) {
			...
			if (!world.isRemote && block instanceof BlockCauldron && itemstack != null && (cauldronMeta = BlockCauldron.func_150027_b(meta)) > 0) {
				...
				Item item = itemstack.getItem();
				if (GOTRecipePoisonWeapon.poisonedToInput.containsKey(item)) {
					Item inputWeapon = GOTRecipePoisonWeapon.poisonedToInput.get(item);
					itemstack.func_150996_a(inputWeapon);
					undyed = true;
				} else if (item instanceof GOTItemPouch && GOTItemPouch.isPouchDyed(itemstack)) {
			...
			if (block == Blocks.brewing_stand && !GOTConfig.enablePotionBrewing) {
				event.setCanceled(true);
				return;
			}
	...
	public void onConfigChanged(ConfigChangedEvent.OnConfigChangedEvent event) {
		if (event.modID.equals("got")) {
			GOTConfig.load();
		}
        if (GOTModChecker.hasNEI()) {
            for (int i = 0; i < NEIModContainer.plugins.size(); ++i) {
                IConfigureNEI iConfigNEI = (IConfigureNEI)NEIModContainer.plugins.get(i);
                if (!iConfigNEI.getClass().equals(NEIGOTIntegratorConfig.class)) continue;
                NEIGOTIntegratorConfig configNEI = (NEIGOTIntegratorConfig)iConfigNEI;
                for (int i2 = 0; i2 < configNEI.hiddenItems.size(); ++i2) {
                    if (!ItemInfo.hiddenItems.contains(configNEI.hiddenItems.get(i2))) continue;
                    ItemInfo.hiddenItems.remove(configNEI.hiddenItems.get(i2));
                }
	...
	public void onLivingUpdate(LivingEvent.LivingUpdateEvent event) {
		...
		if (!world.isRemote && entity.isEntityAlive() && entity.ticksExisted % 20 == 0) {
			boolean flag = true;
			if (entity instanceof GOTBiome.ImmuneToFrost) {
				flag = false;
			}
			if (entity instanceof GOTEntityNPC && ((GOTEntityNPC) entity).isImmuneToFrost) {
				flag = false;
			}
			if (entity instanceof EntityPlayer) {
			...
			if (flag) {
				...
				if ((biome.temperature == 0.0F) && (world.canBlockSeeTheSky(i, j, k2) || entity.isInWater()) && world.getSavedLightValue(EnumSkyBlock.Block, i, j, k2) < 10) {
					...
					for (int l1 = 1; l1 < 4; ++l1) {
						...
						if (material == GOTRegistry.fur) {
							frostProtection += 100;
							continue;
						}
						if(material == GOTRegistry.iceShard) {
							frostProtection += 100;
							continue;
						}
						if(armorMaterial == GOTMaterial.NORTH.toArmorMaterial()) {
							frostProtection += 100;
							continue;
						}
						if(armorMaterial == GOTMaterial.REDKING.toArmorMaterial()) {
							frostProtection += 100;
							continue;
						}
		...
		if (!world.isRemote && entity.isEntityAlive() && entity.ticksExisted % 20 == 0) {
			...
			if (entity instanceof EntityPlayer) {
				flag = !((EntityPlayer) entity).capabilities.isCreativeMode ? 1 : 0;
				i = flag;
			}
			if (entity instanceof GOTBiome.ImmuneToHeat) {
				flag = 0;
			}
			if (flag != 0) {
