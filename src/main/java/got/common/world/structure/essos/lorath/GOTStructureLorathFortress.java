...
public class GOTStructureLorathFortress extends GOTStructureLorathBase {
	...
	public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (int l = 0; l < warriors; ++l) {
			GOTEntityLorathWarrior warrior = new GOTEntityLorathWarrior(world);
			warrior.spawnRidingHorse = false;
			spawnNPCAndSetHome(warrior, world, 0, 1, 0, 24);
		}
		GOTEntityLorathWarlord captain = new GOTEntityLorathWarlord(world);
		captain.spawnRidingHorse = false;
		spawnNPCAndSetHome(captain, world, 0, 1, 4, 8);
		GOTEntityNPCRespawner respawner = new GOTEntityNPCRespawner(world);
		respawner.setSpawnClasses(GOTEntityLorathWarrior.class, GOTEntityLorathArcher.class);
