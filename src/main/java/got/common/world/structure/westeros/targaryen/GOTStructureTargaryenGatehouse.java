...
public class GOTStructureTargaryenGatehouse extends GOTStructureTargaryenBase {

	public GOTStructureTargaryenGatehouse(boolean flag) {
		super(flag);
	}
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (i14 = -2; i14 <= 2; ++i14) {
			for (j13 = 1; j13 <= 7; ++j13) {
				...
				setBlockAndMetadata(world, i14, j13, -1, gateBlock, 10);
				setBlockAndMetadata(world, i14, j13, 1, GOTRegistry.gateIronBars, 10);
			}
		}
		...
		setBlockAndMetadata(world, -2, 10, 2, Blocks.torch, 2);
		setBlockAndMetadata(world, 2, 10, 2, Blocks.torch, 1);
		placeWallBanner(world, 1, 10, -3, GOTItemBanner.BannerType.TARGARYEN, 0);
		placeWallBanner(world, -1, 10, -3, GOTItemBanner.BannerType.TARGARYEN, 0);
		setBlockAndMetadata(world, -3, 12, -3, Blocks.torch, 3);
		setBlockAndMetadata(world, 3, 12, -3, Blocks.torch, 3);
		setBlockAndMetadata(world, -3, 12, 3, Blocks.torch, 4);
		setBlockAndMetadata(world, 3, 12, 3, Blocks.torch, 4);
		placeWallBanner(world, -3, 7, -3, GOTItemBanner.BannerType.TARGARYEN, 2);
		placeWallBanner(world, 3, 7, -3, GOTItemBanner.BannerType.TARGARYEN, 2);
		placeWallBanner(world, 3, 7, 3, GOTItemBanner.BannerType.TARGARYEN, 0);
		placeWallBanner(world, -3, 7, 3, GOTItemBanner.BannerType.TARGARYEN, 0);
		for (i1 = -5; i1 <= 5; ++i1) {
			i22 = Math.abs(i1);
			if (i22 >= 4) {
		...
		for (int i1221 : new int[] { -1, 1 }) {
			j12 = 8;
			int k17 = 0;
			GOTEntityTargaryenSoldier levyman = new GOTEntityTargaryenSoldier(world);
