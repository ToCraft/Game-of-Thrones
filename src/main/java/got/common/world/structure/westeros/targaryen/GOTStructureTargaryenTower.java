...
public class GOTStructureTargaryenTower extends GOTStructureTargaryenBase {
	public boolean generateRoom = true;

	public GOTStructureTargaryenTower(boolean flag) {
		super(flag);
	}
	...
	public boolean generateWithSetRotationAndHeight(World world, Random random, int i, int j, int k, int rotation, int height) {
		...
		if (generateRoom) {
			...
			setBlockAndMetadata(world, 1, doorBase + 1, -1, GOTRegistry.tableTargaryen, 0);
			placeWallBanner(world, 2, doorBase + 4, -1, GOTItemBanner.BannerType.TARGARYEN, 3);
		...
		respawner.setSpawnClass(GOTEntityTargaryenSoldierArcher.class);
