...
public class GOTConquestGrid {
	...
	public static GOTConquestZone getZoneByGridCoords(int i, int k) {
		if (i < 0 || i >= MathHelper.ceiling_float_int((float) GOTGenLayerWorld.imageWidth / (float) MAP_GRID_SCALE)) {
			return dummyZone;
		}
		if (k < 0 || k >= MathHelper.ceiling_float_int((float) GOTGenLayerWorld.imageHeight / (float) MAP_GRID_SCALE)) {
			return dummyZone;
		}
