...
public class GOTEntityAurochs extends EntityCow implements GOTRandomSkinEntity {
	...
	public GOTEntityAurochs(World world) {
		super(world);
		aurochsWidth = 1.5f;
		aurochsHeight = 1.7f;
	...
	public boolean attackEntityFrom(DamageSource damagesource, float f) {
		...
		if (flag && isChild() && (attacker = damagesource.getEntity()) instanceof EntityLivingBase) {
			...
			for (Object element : list) {
				GOTEntityAurochs aurochs;
				Entity entity = (Entity) element;
				if (entity.getClass() != this.getClass() || (aurochs = (GOTEntityAurochs) entity).isChild()) {
					continue;
				}
	...
	public EntityCow createChild(EntityAgeable entity) {
		return new GOTEntityAurochs(worldObj);
	}
