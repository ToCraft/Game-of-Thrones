...
public class GOTEntityAIBanditSteal extends EntityAIBase {
	...
	public void steal() {
		...
		for (int i = 0; i < thefts; ++i) {
			...
			if (this.tryStealItem(inv, GOTItemGem.class)) {
				stolenSomething = true;
				continue;
			}
			if (this.tryStealItem(inv, GOTValuableItems.getToolMaterials())) {
				stolenSomething = true;
				continue;
			}
			if (this.tryStealItem(inv, GOTItemRing.class)) {
				stolenSomething = true;
				continue;
			}
			...
			if (this.tryStealItem(inv, GOTItemPouch.class)) {
				stolenSomething = true;
				continue;
			}
