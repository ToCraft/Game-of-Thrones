...
public abstract class GOTEntityNPC extends EntityCreature implements IRangedAttackMob, GOTRandomSkinEntity {
	...
	public GOTTravellingTraderInfo travellingTraderInfo;
	...
	public void entityInit() {
		...
		if (this instanceof GOTTravellingTrader) {
			travellingTraderInfo = new GOTTravellingTraderInfo((GOTTravellingTrader) (this));
		}
		setupNPCGender();
		setupNPCName();
	...
	public boolean getCanSpawnHere() {
		if ((!spawnsInDarkness || liftSpawnRestrictions || isConquestSpawning && conquestSpawnIgnoresDarkness() || isValidLightLevelForDarkSpawn()) && super.getCanSpawnHere()) {
			if (!liftBannerRestrictions) {
				if (GOTBannerProtection.isProtected(worldObj, this, GOTBannerProtection.forNPC(this), false)) {
					return false;
				}
				if (!isConquestSpawning && GOTEntityNPCRespawner.isSpawnBlocked(this)) {
					return false;
				}
			}
			return true;
	...
	public ItemStack getEquipmentInSlot(int i) {
		if (worldObj.isRemote) {
			if (!initFestiveItems) {
				...
				if (GOT.isNewYear() && festiveRand.nextInt(3) == 0) {
					ItemStack hat;
					if (rand.nextBoolean()) {
	...
	public boolean interact(EntityPlayer entityplayer) {
		if (!worldObj.isRemote && canNPCTalk()) {
			if (questInfo.interact(entityplayer)) {
				return true;
			}
			if (getAttackTarget() == null && speakTo(entityplayer)) {
				return true;
			}
		}
	...
	public void onDeath(DamageSource damagesource) {
		...
		if (!worldObj.isRemote && (this instanceof GOTTradeable || this instanceof GOTUnitTradeable) && !isLegendaryNPC && !(this instanceof GOTTravellingTrader)) {
		...
		if (!worldObj.isRemote && isEntityAlive() && getAttackTarget() == null) {
			...
			if (this instanceof GOTTradeable || this instanceof GOTUnitTradeable || this instanceof GOTMercenary) {
				for (Object element : worldObj.playerEntities) {
					...
					if (container instanceof GOTContainerTrade && ((GOTContainerTrade) container).theTraderNPC == this) {
						guiOpen = true;
						break;
					}
					if (container instanceof GOTContainerUnitTrade && ((GOTContainerUnitTrade) container).theLivingTrader == this) {
						guiOpen = true;
						break;
					}
	...
	public boolean shouldRenderNPCChest() {
		//System.out.print(this.getNPCName() + " - MALE: " + familyInfo.isMale() + "; CHILD: " + isChild() + "\n");
