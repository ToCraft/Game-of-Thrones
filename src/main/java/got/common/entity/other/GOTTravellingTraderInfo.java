...
public class GOTTravellingTraderInfo {
	public GOTEntityNPC theEntity;
	public GOTTravellingTrader theTrader;
	public int timeUntilDespawn = -1;
	public List escortUUIDs = new ArrayList();

	public GOTTravellingTraderInfo(GOTTravellingTrader entity) {
		theEntity = (GOTEntityNPC) (entity);
		theTrader = entity;
	}

	public void onDeath() {
		if (!theEntity.worldObj.isRemote && timeUntilDespawn >= 0) {
			GOTSpeech.messageAllPlayers(theEntity.func_110142_aN().func_151521_b());
			removeEscorts();
		}
	}
		...
		if (!theEntity.worldObj.isRemote) {
			...
			if (timeUntilDespawn == 2400) {
				for (Object player : theEntity.worldObj.playerEntities) {
					GOTSpeech.sendSpeechAndChatMessage((EntityPlayer) player, theEntity, theTrader.getDepartureSpeech());
				}
			}
			if (timeUntilDespawn == 0) {
				...
				GOTSpeech.messageAllPlayersInWorld(theEntity.worldObj, new ChatComponentTranslation("got.travellingTrader.depart", componentName));
				theEntity.setDead();
				removeEscorts();
			}
		}
	...
	public void startVisiting(EntityPlayer entityplayer) {
		...
		if (theEntity.worldObj.playerEntities.size() <= 1) {
			...
			componentName.getChatStyle().setColor(EnumChatFormatting.YELLOW);
			GOTSpeech.messageAllPlayers(new ChatComponentTranslation("got.travellingTrader.arrive", componentName));
		} else {
			componentName = new ChatComponentText(theEntity.getCommandSenderName());
			componentName.getChatStyle().setColor(EnumChatFormatting.YELLOW);
			GOTSpeech.messageAllPlayersInWorld(theEntity.worldObj, new ChatComponentTranslation("got.travellingTrader.arriveMP", componentName, entityplayer.getCommandSenderName()));
		}
		...
		for (int l = 0; l < escorts; ++l) {
			GOTEntityNPC escort = theTrader.createTravellingEscort();
			if (escort == null) {
				continue;
			}
