	...
	public ItemStack transferStackInSlot(EntityPlayer entityplayer, int i) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (i < 4) {
				...
				if (((Slot) inventorySlots.get(0)).isItemValid(itemstack1) && !mergeItemStack(itemstack1, 0, 1, false)) {
					return null;
				}
				if (((Slot) inventorySlots.get(3)).isItemValid(itemstack1) && !mergeItemStack(itemstack1, 3, 4, false)) {
					return null;
				}
			}
