...
public class GOTContainerAnvil extends Container {
	...
	public boolean isRepairMaterial(ItemStack inputItem, ItemStack materialItem) {
		...
		if (item == Items.bow && materialItem.getItem() == Items.string) {
			return true;
		}
		...
		if (item instanceof GOTItemChisel && materialItem.getItem() == Items.iron_ingot) {
			return true;
		}
		...
		if (inputItem == null) {
			...
			if (combinerItem != null) {
				...
				if (combinerItemEnchant != null) {
					...
					if (combinerItemEnchant == GOTEnchantment.fire && GOTRecipePoisonWeapon.poisonedToInput.containsKey(item = inputCopy.getItem())) {
						Item unpoisoned = GOTRecipePoisonWeapon.poisonedToInput.get(item);
