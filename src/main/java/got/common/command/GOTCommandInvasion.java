...
public class GOTCommandInvasion extends CommandBase {
	public List addTabCompletionOptions(ICommandSender sender, String[] args) {
		if (args.length == 1) {
			return GOTCommandInvasion.getListOfStringsMatchingLastWord(args, GOTInvasions.listInvasionNames());
		}
		return null;
	}
	...
	public void processCommand(ICommandSender sender, String[] args) {
		...
		if (args.length >= 1) {
			...
			if (type != null) {
				...
				if (args.length >= 4) {
					posX = GOTCommandInvasion.func_110666_a(sender, posX, args[1]);
					posY = GOTCommandInvasion.func_110666_a(sender, posY, args[2]);
					posZ = GOTCommandInvasion.func_110666_a(sender, posZ, args[3]);
				} else {
					posY += 3.0;
				}
				int size = -1;
				if (args.length >= 5) {
					size = GOTCommandInvasion.parseIntBounded(sender, args[4], 0, 10000);
				}
				...
				GOTCommandInvasion.func_152373_a(sender, this, "got.command.invasion.start", type.invasionName(), invasion.getInvasionSize(), posX, posY, posZ);
				return;
			}
